...
class EMA(pl.Callback):
    ...
    def __init__(self,
        ...
        self.ema_pin_memory = pin_memory if torch.cuda.is_available(
        ) else False  # Only works if CUDA is available
    ...
    def on_train_start(self, trainer: "pl.Trainer",
        ...
        if not self._ema_state_dict_ready and pl_module.global_rank == 0:
            ...
            #     self.ema_device = trainer.device
            if self.ema_device:
                self.ema_state_dict = {
        ...
        elif pl_module.global_rank == 0:
            self.ema_state_dict = {
                k: tensor.to(device=pl_module.device, non_blocking=True)
                for k, tensor in self.ema_state_dict.items()
            }
    ...
    def on_train_batch_end(self, trainer: "pl.Trainer",
        ...
        with torch.no_grad():
            for key, value in self.get_state_dict(pl_module).items():
                ...
                ema_value.copy_(self.decay * ema_value +
                                (1. - self.decay) * value,
                                non_blocking=True)
    ...
    def on_load_checkpoint(self, trainer: "pl.Trainer",
                           pl_module: "pl.LightningModule",
